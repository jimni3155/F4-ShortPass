"""
이연수 면접 평가 (transcript_yeonsu_103.json + resume_yeonsu.json)

Usage:
    python server/tests/evaluation_test_yeonsu.py
"""
import sys
import os
import json
...
async def test_yeonsu_evaluation():
    """이연수 면접 평가"""
    ...
    transcript_path = data_dir / "transcript_yeonsu_103.json"
    resume_path = data_dir / "resume_yeonsu.json"
    ...
    interview_id = transcript.get("interview_id", 103)
    applicant_id = transcript.get("applicant_id", 103)
    ...
    print("\n" + "=" * 80)
    print(" AI 면접 평가 시스템 - 이연수 평가")
    print("=" * 80 + "\n")

    print(f"  Applicant: {resume_data.get('applicant_name', '이연수')}")
    print(f"  Position: {resume_data.get('experience', [{}])[-1].get('position', 'MD')}")
    print(f"  Company: {resume_data.get('experience', [{}])[-1].get('company', '무신사 스탠다드')}")
...
if __name__ == "__main__":
    asyncio.run(test_yeonsu_evaluation())
